...
namespace Core
{
	...
	struct Uuid
	{
		...
		bool isEmpty() const { return ab == 0 && cd == 0; }
	...
	constexpr std::array<uint8_t, 16> fromConstant(const char* str)
	{
...
namespace std {
	template<>
	class hash< Core::Uuid >: public std::unary_function< Core::Uuid, size_t > {
	public:
...
#define UUID_CONSTANT(x) Core::Uuid(Core::fromConstant(x).data())