...
inline PropertyPtr prop(const Node& node, const char* propertyTitle)
{
...
inline size_t propIndex(const Node& node, const char* propertyTitle)
{
	auto prop = find_if(begin(node.properties()), end(node.properties()), property_eq_hash(node.nodeType(), hash(propertyTitle)));
...
template <typename T>
inline T prop(const Node& node, const char* propertyTitle, Frame frame)
{
...
inline ConnectorMetadataPtr connector(const Node& node, std::string connectorTitle)
{
...
inline size_t connectorIndex(const Node& node, std::string connectorTitle)
{
	auto result = find_if(cbegin(node.connectorMetadata()), cend(node.connectorMetadata()), [connectorTitle](auto& metadata)
	{
		return metadata->hash() == hash(connectorTitle.c_str());
